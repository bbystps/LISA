<script>
  (function() {
          ...
          render: function(data, type, row) {
            ...
            if (flag === 'ACTIVE') {
              if (status === 'to deliver') {
                ...
                // NEW: separate acknowledge for returned
                label = 'Acknowledge';
                action = 'ack_returned';
                disabled = false;
              }
    ...
    // --- Primary action handler (Deliver / Acknowledge / Fetch) ---
    $table.on('click', '.table-btn.primary', function() {
      ...
      if (action === "deliver") {
        ...
        $.ajax({
          ...
          cache: false,
          success: function() {
            // Notify robot (optional, mirrors your other actions)
            const payload = JSON.stringify({
              transaction_id: id,
              task: "fetch",
              student_id: studentId,
        ...
        $.ajax({
          ...
          cache: false,
          success: function() {
            // Optional: publish a distinct MQTT event
            const payload = JSON.stringify({
              transaction_id: id,
              task: "ack_returned",
              student_id: studentId,
